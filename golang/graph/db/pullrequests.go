...
type Pullrequest struct {
	...
	Closed      bool   `boil:"closed" json:"closed" toml:"closed" yaml:"closed"`
...
var PullrequestWhere = struct {
	...
	Closed:      whereHelperbool{field: "`pullrequests`.`closed`"},
...
func (o *Pullrequest) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `pullrequests` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `pullrequests` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, pullrequestPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "db: unable to populate default values for pullrequests")
	}
...
func (o *Pullrequest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(pullrequestColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLPullrequestUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`pullrequests`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `pullrequests` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "db: unable to upsert for pullrequests")
	}
	...
	uniqueMap, err = queries.BindMapping(pullrequestType, pullrequestMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "db: unable to retrieve unique values for pullrequests")
	}
