...
var (
	...
	projectcardColumnsWithoutDefault = []string{"id", "project", "issue", "pullrequest"}
	projectcardColumnsWithDefault    = []string{}
...
func (o *Projectcard) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `projectcards` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `projectcards` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, projectcardPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "db: unable to populate default values for projectcards")
	}
...
var mySQLProjectcardUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *Projectcard) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(projectcardColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLProjectcardUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`projectcards`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `projectcards` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "db: unable to upsert for projectcards")
	}
	...
	uniqueMap, err = queries.BindMapping(projectcardType, projectcardMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "db: unable to retrieve unique values for projectcards")
	}
